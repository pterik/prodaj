...
*/
    define('BENDER_BLACK_THEME_VERSION', '1.0.1');
    ...
    if(!OC_ADMIN) {
        ...
        osc_enqueue_style('bender_black-fine-uploader-css', osc_current_web_theme_url('css/ajax-uploader.css'));
    }
...
*/
    ...
    // install options
    if( !function_exists('bender_black_theme_install') ) {
        function bender_black_theme_install() {
            ...
            osc_set_preference('defaultShowAs@all', 'list', 'bender_black_theme');
            osc_set_preference('defaultShowAs@search', 'list');
            osc_reset_preferences();
        }
    }
    // update options
    if( !function_exists('bender_black_theme_update') ) {
        function bender_black_theme_update() {
            //osc_set_preference('version', BENDER_BLACK_THEME_VERSION, 'bender_black_theme');
            osc_delete_preference('default_logo', 'bender_black_theme');

            $logo_prefence = osc_get_preference('logo', 'bender_black_theme');
            $logo_name     = 'bender_black_logo';
            ...
            if( file_exists( $temp_name ) && !$logo_prefence) {
                ...
                @unlink($temp_name);
                osc_set_preference('logo', $logo_name, 'bender_black_theme');
            }
            osc_reset_preferences();
        }
    }
    if(!function_exists('check_install_bender_black_theme')) {
        function check_install_bender_black_theme() {
            ...
            } else if($current_version < BENDER_BLACK_THEME_VERSION){
                bender_black_theme_update();
            }
        }
    }
    ...
    if(!function_exists('bender_black_body_class')) {
        function bender_black_body_class($echo = true){
            /**
            * Print body classes.
            *
    ...
    if(!function_exists('bender_black_add_body_class')) {
        function bender_black_add_body_class($class){
            /**
    ...
    if( !function_exists('logo_header') ) {
        function logo_header() {
             $logo = osc_get_preference('logo','bender_black_theme');
             $html = '<a href="'.osc_base_url().'"><img border="0" alt="' . osc_page_title() . '" src="' . bender_black_logo_url() . '"></a>';
    ...
    /* logo */
    if( !function_exists('bender_black_logo_url') ) {
        function bender_black_logo_url() {
            $logo = osc_get_preference('logo','bender_black_theme');
            if( $logo ) {
    ...
    if( !function_exists('bender_black_draw_item') ) {
        function bender_black_draw_item($class = false,$admin = false, $premium = false) {
    ...
    if( !function_exists('bender_black_show_as') ){
        function bender_black_show_as(){

            $p_sShowAs    = Params::getParam('sShowAs');
            $aValidShowAsValues = array('list', 'gallery');
            if (!in_array($p_sShowAs, $aValidShowAsValues)) {
                $p_sShowAs = bender_black_default_show_as();
            }

            return $p_sShowAs;
        }
    }
    if( !function_exists('bender_black_default_show_as') ){
        function bender_black_default_show_as(){
            return getPreference('defaultShowAs@all','bender_black_theme');
        }
    }
    ...
    if( !function_exists('get_breadcrumb_lang') ) {
        function get_breadcrumb_lang() {
            ...
            $lang['user_items']             = __('Listings', 'bender_black');
            ...
            $lang['login_recover']          = __('Recover password', 'bender_black');
            $lang['login_forgot']           = __('Change password', 'bender_black');
    ...
    if( !function_exists('get_user_menu') ) {
        function get_user_menu() {
            ...
            $options[] = array(
                'name'  => __('Listings', 'bender_black'),
                'url'   => osc_user_list_items_url(),
                'class' => 'opt_items'
            );
            $options[] = array(
                'name' => __('Alerts', 'bender_black'),
...
<script type="text/javascript">
    ...
    function theme_bender_black_actions_admin() {
        ...
        switch( Params::getParam('action_specific') ) {
            case('settings'):
                ...
                osc_set_preference('defaultShowAs@all', Params::getParam('defaultShowAs@all'), 'bender_black_theme');
                osc_set_preference('defaultShowAs@search', Params::getParam('defaultShowAs@all'));

                osc_set_preference('header-728x90',         trim(Params::getParam('header-728x90', false, false, false)),                  'bender_black');
                osc_set_preference('homepage-728x90',       trim(Params::getParam('homepage-728x90', false, false, false)),                'bender_black');
                osc_set_preference('sidebar-300x250',       trim(Params::getParam('sidebar-300x250', false, false, false)),                'bender_black');
                osc_set_preference('search-results-top-728x90',     trim(Params::getParam('search-results-top-728x90', false, false, false)),          'bender_black');
                osc_set_preference('search-results-middle-728x90',  trim(Params::getParam('search-results-middle-728x90', false, false, false)),       'bender_black');
            ...
            case('remove'):
                ...
                if(file_exists( $path ) ) {
                    @unlink( $path );
                    osc_delete_preference('logo','bender_black_theme');
                    osc_reset_preferences();
    ...
    function bender_black_redirect_user_dashboard()
    {
    ...
    function bender_black_delete() {
        Preference::newInstance()->delete(array('s_section' => 'bender_black'));
    }

    osc_add_hook('init', 'bender_black_redirect_user_dashboard', 2);
    osc_add_hook('init_admin', 'theme_bender_black_actions_admin');
    osc_add_hook('theme_delete_bender_black', 'bender_black_delete');
...
if(osc_is_home_page() || osc_is_search_page()){
    bender_black_add_body_class('has-searchbox');
}


function bender_black_sidebar_category_search($catId = null)
{
    $aCategories = array();
    if($catId==null) {
    ...
    bender_black_print_sidebar_category_search($aCategories, $catId);
}

function bender_black_print_sidebar_category_search($aCategories, $current_category = null, $i = 0)
{
    $class = '';
    ...
    if(!isset($c['pk_i_id'])) {
        echo '<ul '.$class.'>';
        if($i==1) {
            echo '<li><a href="'.osc_esc_html(osc_update_search_url(array('sCategory'=>null, 'iPage'=>null))).'">'.__('All categories', 'bender_black')."</a></li>";
        }
    ...
    <ul <?php echo $class;?>>
        <?php if($i==1) { ?>
        <li><a href="<?php echo osc_esc_html(osc_update_search_url(array('sCategory'=>null, 'iPage'=>null))); ?>"><?php _e('All categories', 'bender_black'); ?></a></li>
        <?php } ?>
            <li>
                ...
                <?php bender_black_print_sidebar_category_search($aCategories, $current_category, $i); ?>
            </li>
        <?php if($i==1) { ?>
        <?php } ?>
    </ul>
...
function search_ads_listing_top_fn() {
    if(osc_get_preference('search-results-top-728x90', 'bender_black')!='') {
        echo '<div class="clear"></div>' . PHP_EOL;
        echo '<div class="ads_728">' . PHP_EOL;
        echo osc_get_preference('search-results-top-728x90', 'bender_black');
        echo '</div>' . PHP_EOL;
    }
}
...
function search_ads_listing_medium_fn() {
    if(osc_get_preference('search-results-middle-728x90', 'bender_black')!='') {
        echo '<div class="clear"></div>' . PHP_EOL;
        echo '<div class="ads_728">' . PHP_EOL;
        echo osc_get_preference('search-results-middle-728x90', 'bender_black');
        echo '</div>' . PHP_EOL;
    }
}
